...
from app.models import (  # noqa: E402
    Athlete, AthleteResult, Event, Meet, RelayAthlete, RelayResult, School, SchoolEnrollment,
)
...
def _seed_core_data():
    ...
    enrollments = [
        ...
        SchoolEnrollment(school_id=2, year=2023, enrollment=470),
        SchoolEnrollment(school_id=3, year=2023, enrollment=1190),
    ]
    ...
    meets = [
        ...
        # Girls. The fixture used to be Boys-only, so every gender-conditional
        # branch -- girls' hurdles, the gender split in rankings and dashboards
        # -- went untested. 100 Hurdles is the one event that differs by gender.
        Meet(meet_id=14, host="Alpha Sectional", meet_type="Sectional", meet_num=1, gender="Girls", year=2024),
        Meet(meet_id=15, host="Regional 1", meet_type="Regional", meet_num=1, gender="Girls", year=2024),
        Meet(meet_id=16, host="State Finals", meet_type="State", meet_num=1, gender="Girls", year=2024),
    ]
    ...
    athletes = [
        ...
        Athlete(athlete_id=111, first="Maya", last="Hurdle", school_id=1, gender="Girls", graduation_year=2025),
        Athlete(athlete_id=112, first="Nina", last="Swift", school_id=1, gender="Girls", graduation_year=2026),
        Athlete(athlete_id=113, first="Opal", last="Thrower", school_id=1, gender="Girls", graduation_year=2027),
        Athlete(athlete_id=211, first="Pia", last="Falcon", school_id=2, gender="Girls", graduation_year=2025),
        Athlete(athlete_id=311, first="Quinn", last="Blaze", school_id=3, gender="Girls", graduation_year=2026),
    ]
    ...
    athlete_results = [
        ...
        # Girls, 2024. 100 Hurdles exists only on the girls' side; a tie for 2nd
        # in the 100 gives the scoring code something to split.
        AthleteResult(athlete_id=111, meet_id=14, event="100 Hurdles", result_type="Final", result="15.20", result2=15.2, place=1, grade="SR"),
        AthleteResult(athlete_id=111, meet_id=15, event="100 Hurdles", result_type="Final", result="15.05", result2=15.05, place=2, grade="SR"),
        AthleteResult(athlete_id=111, meet_id=16, event="100 Hurdles", result_type="Prelim", result="15.10", result2=15.1, place=6, grade="SR"),
        AthleteResult(athlete_id=112, meet_id=14, event="100 Meters", result_type="Final", result="12.40", result2=12.4, place=2, grade="JR"),
        AthleteResult(athlete_id=112, meet_id=15, event="100 Meters", result_type="Final", result="12.35", result2=12.35, place=4, grade="JR"),
        AthleteResult(athlete_id=113, meet_id=14, event="Shot Put", result_type="Final", result="34'2\"", result2=410.0, place=3, grade="SO"),
        AthleteResult(athlete_id=211, meet_id=14, event="100 Meters", result_type="Final", result="12.40", result2=12.4, place=2, grade="SR"),
        AthleteResult(athlete_id=211, meet_id=14, event="300 Hurdles", result_type="Final", result="47.10", result2=47.1, place=1, grade="SR"),
        AthleteResult(athlete_id=311, meet_id=14, event="100 Meters", result_type="Final", result="12.10", result2=12.1, place=1, grade="JR"),
    ]
    ...
    relay_results = [
        ...
        RelayResult(school_id=1, meet_id=14, event="4 x 400 Relay", result="4:05.00", result2=245.0, place=1, athlete_names="Maya Hurdle, Nina Swift, Opal Thrower, Relay Four"),
        RelayResult(school_id=2, meet_id=14, event="4 x 400 Relay", result="4:07.50", result2=247.5, place=2, athlete_names="Pia Falcon, Relay Two, Relay Three, Relay Four"),
    ]

    # RelayAthlete links an athlete to a relay leg. Nothing seeded it before,
    # so the table was empty in every test that touched relays.
    relay_athletes = [
        RelayAthlete(relay_id=1, athlete_id=111),
        RelayAthlete(relay_id=1, athlete_id=112),
        RelayAthlete(relay_id=1, athlete_id=113),
        RelayAthlete(relay_id=2, athlete_id=101),
        RelayAthlete(relay_id=2, athlete_id=102),
    ]
...
def app():
    ...
    with app.app_context():
        ...
        (schools, enrollments, meets, athletes, athlete_results, relay_results,
         relay_athletes) = _seed_core_data()
        db.session.add_all(schools + enrollments + meets + athletes + athlete_results
                           + relay_results + relay_athletes)
        db.session.commit()
        _clear_query_caches()
        yield app
        db.session.remove()
