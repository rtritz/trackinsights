...
def _score_event(rows, ascending):
    ...
    if not cleaned:
        return {}
    # Primary: result2 (best first). Ties on the exact same mark are grouped below.
    ...
    while i < n:
        ...
        scoring_slots = [p for p in range(place, place + tie_size) if p in REGIONAL_POINTS]
        pts_value = (sum(REGIONAL_POINTS[p] for p in scoring_slots) / tie_size) if scoring_slots else 0
        if pts_value:
