...
`from backend.queries import X` and `queries.X` work exactly as before -- callers
did not have to change, and new code can import from the specific module instead.

    shared.py            helpers every feature uses
    school_dashboard.py  school dashboards and statewide rankings
...
`from backend.queries import your_function` will not find it. New code can also
import straight from the module (`from backend.queries.athletes import ...`),
