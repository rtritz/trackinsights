...
From web/:
    python -m app.jobs.precompute_dashboard_v4
    python -m app.jobs.precompute_dashboard_v4 --limit 25    # a quick subset
    python -m app.jobs.precompute_dashboard_v4 --check       # is it stale?
"""
import json
import os
import sys
...
from app.services import dashboard_v4 as svc  # noqa: E402
...
def check():
    ...
    if status['stale']:
        print('dashboard cache: STALE -- built %s, results have changed since'
              % status['generated_at'])
        print('  rebuild: python -m app.jobs.precompute_dashboard_v4')
        return 1
