/* State Qualifiers.
 *
 * Moved out of insights/state-qualifiers.html, where it sat as 763 lines inside a <script> tag.
 *
 * This page is configured by its route -- the API it calls and the wording it
 * shows differ between the regional and state versions. Those settings used to
 * be templated straight into `const` declarations, which is what kept the code
 * stuck in the template. They now arrive as JSON in a
 * <script type="application/json" id="page-config"> block, read below.
 *
 * If you add a setting: add it to the JSON block in the template and read it
 * here. Do not template values into JavaScript.
 */

document.addEventListener('DOMContentLoaded', () => {
  const pageConfig = JSON.parse(
      document.getElementById('page-config').textContent);
  const apiStatusUrl = pageConfig.apiStatusUrl;
  const apiDetailUrl = pageConfig.apiDetailUrl;
  const loadErrorMessage = pageConfig.loadErrorMessage;
  const statusErrorMessage = pageConfig.statusErrorMessage;
  const pendingText = pageConfig.pendingText;
  // The jump-to-event chip strip is shared with the other qualifiers
  // page; it lives in js/event-jump.js, loaded before this file.
  const { eventSlug, shortEventLabel, initEventJump, resetEventJump }
    = window.EventJump;

  const params = new URLSearchParams(window.location.search);
  const parsedYear = Number.parseInt(params.get('year') || '2026', 10);
  const selectedYear = Number.isInteger(parsedYear) && parsedYear >= 2000 ? parsedYear : 2026;

  const initialView = (params.get('view') === 'combined') ? 'combined' : 'qualifiers';

  const state = {
    year: selectedYear,
    gender: 'Boys',
    view: initialView,
    statusPayload: null,
    autoSwitchTried: false,
    regionalHosts: {},
    regionalsLoaded: 0,
    combinedCache: {},
  };

  const yearSelect = document.getElementById('year-select');
  const genderGroup = document.getElementById('gender-group');
  const statusLine = document.getElementById('status-line');
  // Removed loading indicators
  const resultsOuter = document.getElementById('results-outer');
  const detailLoading = document.getElementById('detail-loading');
  const detailContent = document.getElementById('detail-content');
  const topListBtn = document.getElementById('top-list-btn');
  const genders = ['Boys', 'Girls'];

  yearSelect.value = String(state.year);

  yearSelect.addEventListener('change', () => {
  const nextYear = Number.parseInt(yearSelect.value, 10);
    if (!Number.isInteger(nextYear) || nextYear < 2000) {
      yearSelect.value = String(state.year);
      return;
    }

    state.year = nextYear;
    state.autoSwitchTried = false;
    state.combinedCache = {};
  const nextUrl = new URL(window.location.href);
    nextUrl.searchParams.set('year', String(state.year));
    window.history.replaceState({}, '', nextUrl.toString());
    fetchStatus();
  });

  function setBanner(text, tone = 'info') {
    if (!text) {
      statusLine.classList.add('hidden');
      statusLine.textContent = '';
      return;
    }

    statusLine.classList.remove('hidden');
    if (tone === 'warning') {
      statusLine.className = 'text-sm text-base-content/50 px-1';
    } else if (tone === 'error') {
      statusLine.className = 'text-sm text-red-700 bg-red-50 border border-red-200 rounded-full px-3 py-1 inline-flex items-center';
    } else {
      statusLine.className = 'text-sm text-base-content/60 px-1';
    }
    statusLine.textContent = text;
  }

  function renderGenderButtons() {
    genderGroup.innerHTML = '';
    genders.forEach((gender) => {
      const btn = document.createElement('button');
      btn.type = 'button';
      btn.className = `btn btn-sm rounded-full ${gender === state.gender ? 'btn-primary' : 'btn-outline'}`;
      btn.textContent = gender;
      btn.addEventListener('click', () => {
        if (state.gender === gender) return;
        state.gender = gender;
        state.autoSwitchTried = true;
        renderGenderButtons();
        fetchStatus();
      });
      genderGroup.appendChild(btn);
    });
  }

  function clearResults() {
    if (resultsOuter) resultsOuter.classList.add('hidden');
    detailContent.classList.add('hidden');
    detailContent.innerHTML = '';
    resetEventJump();
  }

  function renderPending() {
    clearResults();
    setBanner('Pending', 'warning');
  }

  function renderDetail(payload) {
  const context = payload.context || {};
  const events = Array.isArray(payload.events) ? payload.events : [];

    if (context.status !== 'ready') {
      renderPending();
      return;
    }

    if (!events.length) {
      clearResults();
      setBanner('No qualifier data found for this state meet.', 'warning');
      return;
    }

    initEventJump(events);

    setBanner('', 'info');
    if (resultsOuter) resultsOuter.classList.remove('hidden');
    detailContent.classList.remove('hidden');
    detailContent.innerHTML = '';

    events.forEach((eventBlock, index) => {
      // Insert header above the 100 Meters event only, for Boys or Girls 2026 qualifiers view
      if (
        index === 0 &&
        eventBlock.event === '100 Meters' &&
        state.year === 2026 &&
        (state.gender === 'Boys' || state.gender === 'Girls') &&
        state.view === 'qualifiers'
      ) {
        const qualifiersHeader = document.createElement('h2');
        qualifiersHeader.className = 'text-base sm:text-lg font-semibold text-primary mb-2 text-left';
        qualifiersHeader.textContent = `2026 ${state.gender} Unofficial State Qualifiers`;
        detailContent.appendChild(qualifiersHeader);
      }

      const section = document.createElement('section');
      section.className = 'space-y-2';
      const chipKey = `${eventSlug(eventBlock.event)}-${index}`;
      section.id = `event-${chipKey}`;
      section.dataset.chipKey = chipKey;

      const headerWrap = document.createElement('div');
      headerWrap.className = 'flex flex-wrap items-center gap-2 relative';

      const header = document.createElement('h3');
      header.className = 'text-base font-semibold text-base-content';
      header.textContent = eventBlock.event;
      headerWrap.appendChild(header);

      if (eventBlock.standard_mark) {
        const standard = document.createElement('span');
        standard.className = 'badge badge-sm text-xs font-semibold border-emerald-600/40 bg-emerald-50 text-emerald-700';
        standard.textContent = `Standard: ${eventBlock.standard_mark}`;
        headerWrap.appendChild(standard);
      }

      // Up arrow button at top right of section header
      const upArrowBtn = document.createElement('button');
      upArrowBtn.type = 'button';
      upArrowBtn.setAttribute('aria-label', 'Back to top');
      upArrowBtn.setAttribute('title', 'Back to top');
      upArrowBtn.className = 'up-arrow-btn absolute right-0 top-0 bg-white border border-base-300 rounded-full p-0.5 shadow-sm hover:border-primary hover:bg-primary/10 focus:bg-primary/20 focus:border-primary transition-colors outline-none cursor-pointer';
      upArrowBtn.innerHTML = `
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
          <path fill-rule="evenodd" d="M10.707 3.293a1 1 0 00-1.414 0l-6 6a1 1 0 101.414 1.414L9 6.414V17a1 1 0 102 0V6.414l4.293 4.293a1 1 0 001.414-1.414l-6-6z" clip-rule="evenodd" />
        </svg>
      `;
      upArrowBtn.addEventListener('click', () => {
        window.scrollTo({ top: 0, behavior: 'smooth' });
      });
      headerWrap.appendChild(upArrowBtn);

      section.appendChild(headerWrap);

      const qualifiers = Array.isArray(eventBlock.qualifiers) ? eventBlock.qualifiers : [];
      if (!qualifiers.length) {
        const empty = document.createElement('p');
        empty.className = 'text-sm text-base-content/60';
        empty.textContent = 'No qualifiers available.';
        section.appendChild(empty);
        detailContent.appendChild(section);
        return;
      }

      const wrap = document.createElement('div');
      wrap.className = 'overflow-x-auto border border-base-200 rounded-lg';
      const isRelayEvent = String(eventBlock.event || '').startsWith('4 x ');
      const nameHeader = isRelayEvent ? 'School' : 'Name';
      const gradeHeader = isRelayEvent ? '' : 'Grade';
      wrap.innerHTML = `
        <table class="table table-xs sm:table-sm w-full bg-white">
          <thead>
            <tr>
              <th class="w-10"></th>
              <th class="text-left pl-1">${nameHeader}</th>
              <th>${gradeHeader}</th>
              <th>School</th>
              <th>Result</th>
              <th>Place</th>
              <th>Regional Host</th>
            </tr>
          </thead>
          <tbody></tbody>
        </table>
      `;

      const tbody = wrap.querySelector('tbody');
      qualifiers.forEach((row, index) => {
        const tr = document.createElement('tr');
        const displayName = row.name || row.school || '-';
        const metStandard = Boolean(row.met_standard) || row.qualifier_type === 'standard';
        const linkClass = 'text-inherit hover:underline';
        let linkedName = displayName;
        if (!row.is_placeholder && displayName !== '-') {
          if (isRelayEvent && row.school_id) {
            linkedName = `<a href="/school-dashboard/${row.school_id}" class="${linkClass}">${displayName}</a>`;
          } else if (!isRelayEvent && row.athlete_id) {
            linkedName = `<a href="/athlete-dashboard/${row.athlete_id}" class="${linkClass}">${displayName}</a>`;
          }
        }
        let nameText = linkedName;
        if (row.qualifier_type === 'fill' || row.is_callback) {
          nameText = `${linkedName} <span class="text-blue-600 font-semibold">(callback)</span>`;
        }
        const resultText = metStandard
          ? `${row.result || ''} <span class="text-emerald-700 font-bold ml-1">STAN</span>`
          : `${row.result || ''}`;
        const schoolDisplay = row.school || '';
        const schoolCell = (!row.is_placeholder && row.school_id && schoolDisplay && schoolDisplay !== '-')
          ? `<a href="/school-dashboard/${row.school_id}" class="${linkClass}">${schoolDisplay}</a>`
          : schoolDisplay;
        tr.innerHTML = `
          <td class="w-10 pr-1 font-semibold">${index + 1}</td>
          <td class="pl-1">${nameText}</td>
          <td>${row.grade || ''}</td>
          <td>${schoolCell}</td>
          <td>${resultText}</td>
          <td>${row.place || ''}</td>
          <td>${row.sectional_host || ''}</td>
        `;
        tbody.appendChild(tr);
      });

      section.appendChild(wrap);
      detailContent.appendChild(section);
    });
  }

  async function fetchStatus() {
    clearResults();
    statusLine.classList.add('hidden');
    statusLine.textContent = '';

    let spinnerTimeout = setTimeout(() => {
      detailLoading.classList.remove('hidden');
    }, 500);

    try {
      const response = await fetch(`${apiStatusUrl}?gender=${encodeURIComponent(state.gender)}&year=${state.year}`);
      if (!response.ok) {
        throw new Error(statusErrorMessage);
      }
      const payload = await response.json();
      state.statusPayload = payload;
      state.regionalHosts = payload.regional_hosts || {};
      state.regionalsLoaded = Number(payload.completed_regionals || (payload.loaded_regionals || []).length || 0);
      renderViewTabs();

      // If the user has selected 'combined' but no regionals are loaded for this
      // gender/year, fall back to the qualifiers view so they always see content.
      if (state.view === 'combined' && state.regionalsLoaded <= 0) {
        setView('qualifiers', { fetch: false });
      }

      if (state.view === 'combined') {
        clearTimeout(spinnerTimeout);
        detailLoading.classList.add('hidden');
        await fetchTopList();
        return;
      }

      if (payload.status !== 'ready') {
        if (!state.autoSwitchTried) {
          state.autoSwitchTried = true;
          const otherGender = state.gender === 'Boys' ? 'Girls' : 'Boys';
          try {
            const otherResp = await fetch(`${apiStatusUrl}?gender=${encodeURIComponent(otherGender)}&year=${state.year}`);
            if (otherResp.ok) {
              const otherPayload = await otherResp.json();
              if (otherPayload.status === 'ready') {
                state.gender = otherGender;
                renderGenderButtons();
                clearTimeout(spinnerTimeout);
                detailLoading.classList.add('hidden');
                await fetchStatus();
                return;
              }
            }
          } catch (_) { /* ignore probe failure, fall through to Pending */ }
        }
        clearTimeout(spinnerTimeout);
        detailLoading.classList.add('hidden');
        renderPending(payload);
        return;
      }

      const detailResponse = await fetch(`${apiDetailUrl}?gender=${encodeURIComponent(state.gender)}&year=${state.year}`);
      if (!detailResponse.ok) {
        throw new Error(loadErrorMessage);
      }
      const detailPayload = await detailResponse.json();
      clearTimeout(spinnerTimeout);
      detailLoading.classList.add('hidden');
      renderDetail(detailPayload);
    } catch (error) {
      clearTimeout(spinnerTimeout);
      detailLoading.classList.add('hidden');
      clearResults();
      renderViewTabs();
      setBanner(error.message || loadErrorMessage, 'error');
    }
  }

  renderGenderButtons();
  renderViewTabs();
  fetchStatus();

  // ---- Combined Regional Rankings (top list across all 8 regionals) ----
  const topListState = { requestId: 0 };

  function buildTopListEventTable(eventBlock, rows, expanded) {
  const wrap = document.createElement('div');
    wrap.className = 'overflow-x-auto relative';
  const isRelayEvent = String(eventBlock.event || '').startsWith('4 x ');
  const nameHeader = isRelayEvent ? 'School' : 'Name';
  const gradeHeader = isRelayEvent ? '' : 'Grade';
    wrap.innerHTML = `
      <table class="table table-xs sm:table-sm w-full bg-white">
        <thead>
          <tr>
            <th class="w-10"></th>
            <th class="text-left pl-1">${nameHeader}</th>
            <th>${gradeHeader}</th>
            <th>School</th>
            <th>Result</th>
            <th>Place</th>
            <th>Regional Host</th>
          </tr>
        </thead>
        <tbody></tbody>
      </table>
    `;
  const tbody = wrap.querySelector('tbody');

    // Tie-aware ranks across the full list so they're consistent regardless of expansion.
  const ranks = new Array(rows.length);
    let lastVal = null;
    let lastRank = 0;
    for (let i = 0; i < rows.length; i++) {
      const v = rows[i]?.result2;
      if (i > 0 && v !== null && v !== undefined && v === lastVal) {
        ranks[i] = lastRank;
      } else {
        ranks[i] = i + 1;
        lastRank = i + 1;
        lastVal = v;
      }
    }

  const visibleRows = expanded ? rows : rows.slice(0, 10);
  const linkClass = 'text-inherit hover:underline';
    visibleRows.forEach((row, idx) => {
      const tr = document.createElement('tr');
      const rank = ranks[idx];
      const displayName = row.name || row.school || '-';
      const metStandard = Boolean(row.met_standard) || row.qualifier_type === 'standard';
      let nameText = displayName;
      if (!row.is_placeholder && displayName !== '-') {
        if (isRelayEvent && row.school_id) {
          nameText = `<a href="/school-dashboard/${row.school_id}" class="${linkClass}">${displayName}</a>`;
        } else if (!isRelayEvent && row.athlete_id) {
          nameText = `<a href="/athlete-dashboard/${row.athlete_id}" class="${linkClass}">${displayName}</a>`;
        }
      }
      if (row.is_placeholder) {
        tr.classList.add('text-orange-600', 'bg-orange-50');
      }
      const resultText = metStandard
        ? `${row.result || ''} <span class="text-emerald-700 font-bold ml-1">STAN</span>`
        : `${row.result || ''}`;
      const schoolDisplay = row.school || '';
      const schoolCell = (!row.is_placeholder && row.school_id && schoolDisplay && schoolDisplay !== '-')
        ? `<a href="/school-dashboard/${row.school_id}" class="${linkClass}">${schoolDisplay}</a>`
        : schoolDisplay;
      const regionalHost = (row.regional_num != null && state.regionalHosts)
        ? (state.regionalHosts[row.regional_num] || state.regionalHosts[String(row.regional_num)] || '')
        : '';
      tr.innerHTML = `
        <td class="w-10 pr-1 font-semibold">${rank}</td>
        <td class="pl-1">${nameText}</td>
        <td>${row.grade || ''}</td>
        <td>${schoolCell}</td>
        <td>${resultText}</td>
        <td>${row.place || ''}</td>
        <td>${regionalHost}</td>
      `;
      tbody.appendChild(tr);
    });
    return wrap;
  }

  function renderTopList(payload) {
  const events = payload.events || [];

    setBanner('', 'info');

    if (!events.length) {
      detailContent.classList.add('hidden');
      detailContent.innerHTML = '';
      resetEventJump();
      setBanner('No data available yet for any regional.', 'warning');
      return;
    }

    if (resultsOuter) resultsOuter.classList.remove('hidden');
    detailContent.classList.add('hidden');
    detailContent.innerHTML = '';
    initEventJump(events);
    detailContent.classList.remove('hidden');

    events.forEach((eventBlock, index) => {

      // Insert header above the 100 Meters event only, for Boys or Girls 2026 combined view
      if (
        index === 0 &&
        eventBlock.event === '100 Meters' &&
        state.year === 2026 &&
        (state.gender === 'Boys' || state.gender === 'Girls') &&
        state.view === 'combined'
      ) {
        const combinedHeader = document.createElement('h2');
        combinedHeader.className = 'text-base sm:text-lg font-semibold text-primary mb-2 text-left';
        combinedHeader.textContent = `2026 ${state.gender} Combined Regional Results`;
        detailContent.appendChild(combinedHeader);
      }

      const section = document.createElement('section');
      section.className = 'space-y-2';
      const chipKey = `${eventSlug(eventBlock.event)}-${index}`;
      section.id = `event-${chipKey}`;
      section.dataset.chipKey = chipKey;

      const headerWrap = document.createElement('div');
      headerWrap.className = 'flex flex-wrap items-center gap-2 relative';

      const header = document.createElement('h3');
      header.className = 'text-base font-semibold text-base-content';
      header.textContent = eventBlock.event;
      headerWrap.appendChild(header);

      if (eventBlock.standard_mark) {
        const standard = document.createElement('span');
        standard.className = 'badge badge-sm text-xs font-semibold border-emerald-600/40 bg-emerald-50 text-emerald-700';
        standard.textContent = `Standard: ${eventBlock.standard_mark}`;
        headerWrap.appendChild(standard);
      }

      const upArrowBtn = document.createElement('button');
      upArrowBtn.type = 'button';
      upArrowBtn.setAttribute('aria-label', 'Back to top');
      upArrowBtn.setAttribute('title', 'Back to top');
      upArrowBtn.className = 'up-arrow-btn absolute right-0 top-0 bg-white border border-base-300 rounded-full p-0.5 shadow-sm hover:border-primary hover:bg-primary/10 focus:bg-primary/20 focus:border-primary transition-colors outline-none cursor-pointer';
      upArrowBtn.innerHTML = `
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
          <path fill-rule="evenodd" d="M10.707 3.293a1 1 0 00-1.414 0l-6 6a1 1 0 101.414 1.414L9 6.414V17a1 1 0 102 0V6.414l4.293 4.293a1 1 0 001.414-1.414l-6-6z" clip-rule="evenodd" />
        </svg>
      `;
      upArrowBtn.addEventListener('click', () => {
        window.scrollTo({ top: 0, behavior: 'smooth' });
      });
      headerWrap.appendChild(upArrowBtn);

      section.appendChild(headerWrap);

      const rows = Array.isArray(eventBlock.qualifiers) ? eventBlock.qualifiers : [];
      if (!rows.length) {
        const empty = document.createElement('p');
        empty.className = 'text-sm text-base-content/60';
        empty.textContent = 'No entries available.';
        section.appendChild(empty);
        detailContent.appendChild(section);
        return;
      }

      let expanded = false;
      const wrap = document.createElement('div');
      wrap.className = 'overflow-x-auto border border-base-200 rounded-lg';
      const tableHolder = document.createElement('div');
      tableHolder.appendChild(buildTopListEventTable(eventBlock, rows, expanded));
      wrap.appendChild(tableHolder);

      if (rows.length > 10) {
        const footer = document.createElement('div');
        footer.className = 'flex justify-center border-t border-base-200 bg-base-100/40 px-2 py-2';
        const showAllBtn = document.createElement('button');
        showAllBtn.type = 'button';
        showAllBtn.className = 'btn btn-xs btn-ghost text-primary';
        showAllBtn.textContent = `Show All (${rows.length})`;
        showAllBtn.addEventListener('click', () => {
          expanded = !expanded;
          tableHolder.innerHTML = '';
          tableHolder.appendChild(buildTopListEventTable(eventBlock, rows, expanded));
          showAllBtn.textContent = expanded ? 'Collapse' : `Show All (${rows.length})`;
        });
        footer.appendChild(showAllBtn);
        wrap.appendChild(footer);
      }

      section.appendChild(wrap);
      detailContent.appendChild(section);
    });
  }

  async function fetchTopList() {
    topListState.requestId += 1;
  const requestId = topListState.requestId;
    clearResults();
    statusLine.classList.add('hidden');
    statusLine.textContent = '';

  const cacheKey = `${state.gender}|${state.year}`;
  const cached = state.combinedCache[cacheKey];
    if (cached) {
      renderTopList(cached);
      detailLoading.classList.add('hidden');
      return;
    }
    let spinnerTimeout = setTimeout(() => {
      detailLoading.classList.remove('hidden');
    }, 500);
    try {
      const response = await fetch(`/api/regional-qualifiers/top-list?gender=${encodeURIComponent(state.gender)}&year=${state.year}&source=results`);
      if (requestId !== topListState.requestId) {
        clearTimeout(spinnerTimeout);
        detailLoading.classList.add('hidden');
        return;
      }
      if (!response.ok) {
        throw new Error('Unable to load combined regional results.');
      }
      const payload = await response.json();
      if (requestId !== topListState.requestId) {
        clearTimeout(spinnerTimeout);
        detailLoading.classList.add('hidden');
        return;
      }
      state.combinedCache[cacheKey] = payload;
      clearTimeout(spinnerTimeout);
      detailLoading.classList.add('hidden');
      renderTopList(payload);
    } catch (error) {
      if (requestId !== topListState.requestId) {
        clearTimeout(spinnerTimeout);
        detailLoading.classList.add('hidden');
        return;
      }
      detailContent.classList.add('hidden');
      detailContent.innerHTML = '';
      resetEventJump();
      clearTimeout(spinnerTimeout);
      detailLoading.classList.add('hidden');
      setBanner(error.message || 'Unable to load combined regional results.', 'error');
    }
  }

  function renderViewTabs() {
  const combinedAvailable = state.regionalsLoaded > 0;

    if (topListBtn) {
      topListBtn.classList.toggle('hidden', !combinedAvailable);
      topListBtn.disabled = !combinedAvailable;
      const isActive = state.view === 'combined';
      topListBtn.classList.toggle('btn-outline', !isActive);
      topListBtn.classList.toggle('btn-primary', isActive);
      topListBtn.classList.toggle('text-primary', !isActive);
      topListBtn.classList.toggle('text-primary-content', isActive);
      topListBtn.setAttribute('aria-pressed', isActive ? 'true' : 'false');

      const labelText = isActive
        ? 'Back to Unofficial State Qualifiers'
        : 'Combined Regional Results';
      topListBtn.title = labelText;
      topListBtn.setAttribute('aria-label', labelText);
      const labelEl = document.getElementById('top-list-btn-label');
      if (labelEl) labelEl.textContent = labelText;
      const iconEl = document.getElementById('top-list-btn-icon');
      if (iconEl) {
        iconEl.innerHTML = isActive
          ? '<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" d="M9.707 14.707a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 1.414L7.414 9H15a1 1 0 110 2H7.414l2.293 2.293a1 1 0 010 1.414z" clip-rule="evenodd" /></svg>'
          : '<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M8 6h13" /><path d="M8 12h13" /><path d="M8 18h13" /><path d="M4 5l1 1L4 7" /><path d="M4 11l1 1-1 1" /><path d="M4 17l1 1-1 1" /></svg>';
      }
    }
  }

  function setView(view, { fetch = true } = {}) {
    if (view !== 'qualifiers' && view !== 'combined') return;
    if (state.view === view) return;
    state.view = view;
  const nextUrl = new URL(window.location.href);
    if (view === 'combined') {
      nextUrl.searchParams.set('view', 'combined');
    } else {
      nextUrl.searchParams.delete('view');
    }
    window.history.replaceState({}, '', nextUrl.toString());
    renderViewTabs();
    if (fetch) {
      fetchStatus();
    }
  }

  if (topListBtn) {
    topListBtn.addEventListener('click', () => {
      if (topListBtn.disabled) return;
      setView(state.view === 'combined' ? 'qualifiers' : 'combined');
    });
  }
});
