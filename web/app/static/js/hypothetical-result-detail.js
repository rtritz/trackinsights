/* Hypothetical Result Detail
 *
 * Moved out of insights/hypothetical-result-detail.html, where it sat as 430 lines inside a <script> tag.
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function renderRankings(rankings, container, context = {}) {
    ...
    configs.forEach((config, index) => {
      ...
      if (!rankingData && config.key !== 'overall') return;
      // Omit grade-level rank card when no grade was specified
      if (config.key === 'same_grade' && !gradeLevel) return;
      // Omit enrollment rank card when no enrollment was specified
      if (config.key === 'like_schools' && !enrollment) return;
