/* Percentiles
 *
 * Moved out of insights/percentiles.html, where it sat as 694 lines inside a <script> tag.
...
const ALL_OPTION_VALUE = '__ALL__';
  ...
  const createChip = (value, container, options = {}) => {
    const { enforceMinOne = false, selectionMode = 'multi', label, isAll = false } = options;
    const chip = document.createElement('button');
    chip.type = 'button';
    chip.className = chipBaseClasses;
    chip.dataset.value = value;
    chip.dataset.selected = 'false';
    chip.dataset.isAll = isAll ? 'true' : 'false';
    ...
    chip.addEventListener('click', () => {
      if (selectionMode === 'single') {
        ...
        Array.from(container.querySelectorAll('.chip-btn')).forEach(other => {
          setChipSelected(other, other === chip);
        });
        queueFetch();
        return;
      }
  ...
  const renderChipGroup = (values, container, options = {}) => {
    ...
    const { includeAll = false, allLabel = 'All' } = options;
    container.innerHTML = '';
    if (includeAll) {
      createChip(ALL_OPTION_VALUE, container, { ...options, label: allLabel, isAll: true });
    }
  ...
  const setSingleSelection = (container, targetValue, fallbackValue = ALL_OPTION_VALUE) => {
    if (!container) {
      return;
    }
    const chips = Array.from(container.querySelectorAll('.chip-btn'));
    if (!chips.length) {
      return;
    }
    let matched = false;
    chips.forEach(chip => {
      const isMatch = chip.dataset.value === targetValue;
      setChipSelected(chip, isMatch);
      if (isMatch) {
        matched = true;
      }
    });
    if (!matched) {
      const fallbackChip = chips.find(chip => chip.dataset.value === fallbackValue) || chips[0];
      chips.forEach(chip => setChipSelected(chip, chip === fallbackChip));
    }
  };
  ...
  const normalizeSingleSelection = (values = []) => {
    if (!Array.isArray(values) || !values.length || values.includes(ALL_OPTION_VALUE)) {
      return [];
    }
    return [values[0]];
  };
  ...
  const collectQueryParams = () => {
    return {
      ...
      meet_types: normalizeSingleSelection(getSelectedChips(meetChipGroup)),
      grade_levels: normalizeSingleSelection(getSelectedChips(gradeChipGroup)),
      years: normalizeSingleSelection(getSelectedChips(yearChipGroup)),
  ...
  const renderResults = (payload) => {
    if (!state.selectedEvent) {
      ...
      if (sampleSizeDisplay) {
        sampleSizeDisplay.classList.add('hidden');
      }
      return;
    }
    ...
    const metricColumns = state.columns.filter(column => !['Gender', 'Event', 'sample_count'].includes(column));
      ...
      resultsBody.innerHTML = activeMetrics
        .map(metric => {
          const cells = genders
            ...
            .join('');
          return `<tr class="transition-colors duration-150 hover:bg-primary/5"><td class="font-semibold text-xs uppercase tracking-wide">${metric}</td>${cells}</tr>`;
        })
        .join('');

      // Display sample sizes
      if (sampleSizeDisplay && sampleSizeContent) {
        const sampleParts = genders.map(gender => {
          const source = rowByGender.get(gender);
          const count = source?.sample_count || 0;
          return `<span class="inline-block"><strong>${gender}:</strong> ${count.toLocaleString()} results</span>`;
        });
        if (sampleParts.length) {
          sampleSizeContent.innerHTML = sampleParts.join('<span class="mx-2 text-gray-300">|</span>');
          sampleSizeDisplay.classList.remove('hidden');
        } else {
          sampleSizeDisplay.classList.add('hidden');
        }
      }
  ...
  const resetFilters = () => {
    ...
    syncEventSelection();
    setSingleSelection(meetChipGroup, ALL_OPTION_VALUE);
    setSingleSelection(gradeChipGroup, ALL_OPTION_VALUE);
    setSingleSelection(yearChipGroup, ALL_OPTION_VALUE);
  ...
  copyQueryBtn.addEventListener('click', async () => {
    ...
    const path = '/insights/percentiles';
  ...
  renderChipGroup(options.meet_types || [], meetChipGroup, { selectionMode: 'single', includeAll: true });
  const meetDefaultValue = initialFilters.meet_types.length ? initialFilters.meet_types[0] : ALL_OPTION_VALUE;
  setSingleSelection(meetChipGroup, meetDefaultValue, ALL_OPTION_VALUE);

  renderChipGroup(options.grade_levels || [], gradeChipGroup, { selectionMode: 'single', includeAll: true });
  const gradeDefaultValue = initialFilters.grade_levels.length ? initialFilters.grade_levels[0] : ALL_OPTION_VALUE;
  setSingleSelection(gradeChipGroup, gradeDefaultValue, ALL_OPTION_VALUE);
      const orderedYears = orderYears(options.years || []);
  renderChipGroup(orderedYears, yearChipGroup, { selectionMode: 'single', includeAll: true });
  const yearDefaultValue = initialFilters.years.length ? initialFilters.years[0] : ALL_OPTION_VALUE;
  setSingleSelection(yearChipGroup, yearDefaultValue, ALL_OPTION_VALUE);
