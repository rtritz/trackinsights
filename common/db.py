...
class Database:
	...
	def insert_school(self, name, team_name, type, nickname, address, city, zip, myihsaa_id=None, commit=True):
		query = "INSERT INTO school (school_name, team_name, school_type, nickname, address, city, zip, myihsaa_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
		parameters = (name, team_name, type, nickname, address, city, zip, myihsaa_id)
		self._execute_write(query, parameters)

		if commit:
			self.conn.commit()

	def get_school_id_by_myihsaa_id(self, myihsaa_id):
		"""Look up a school by its stable myIHSAA/IHSAA "SchoolId" UUID --
		use this before falling back to name matching, since IHSAA's export
		has renamed the same school across years (e.g. "Trinity School at
		Greenlawn" -> "Trinity Academy at Greenlawn" -> "Trinity Academy
		South Bend"), which silently created duplicate rows under exact
		name-only matching.
		"""
		if not myihsaa_id:
			return None
		query = "SELECT school_id from school where myihsaa_id = ?"
		parameters = (myihsaa_id,)
		df = pd.read_sql_query(query, self.conn, params=parameters)
		if df.size == 1:
			return int(df.iloc[0, 0])
		else:
	...
	def update_school_myihsaa_id(self, school_id, myihsaa_id, commit=True):
		query = "UPDATE school SET myihsaa_id = ? WHERE school_id = ?"
		parameters = (myihsaa_id, school_id)
		self._execute_write(query, parameters)
