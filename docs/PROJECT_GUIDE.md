...
# The dashboard cache is not in git -- fetch it from the Release before touching
# the live site, so a failed download aborts while ~/mysite is still intact.
# -f makes curl exit non-zero on a 404 instead of writing the error page to disk.
CACHE_URL=https://github.com/<user>/trackinsights/releases/download/cache-latest/dashboard_cache.db
curl -fsSL "$CACHE_URL" -o "$TEMP/web/data/dashboard_cache.db"   || { echo "could not download dashboard_cache.db -- aborting"; exit 1; }

# A truncated or HTML-error download is still a file. Check it is really SQLite
